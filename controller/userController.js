...
const getUser = async (req,res) => {
    ...
    try {
        ...
        const acount = await acountSchema.find({_id:verify._id})
        acount?res.status(200).json({acount:acount}):res.send('Wrong acount')
    } catch (error) {
        res.status(500).json(error.message)
    }
...
const getAvatar=async (req,res)=>{
    res.sendFile(path.join('D:/shoppingbe/public/'+req.params.address));
}

const deleteAvatar= async (req,res)=>{
    const filename=req.params.address;
    const directory='D:/shoppingbe/public/';

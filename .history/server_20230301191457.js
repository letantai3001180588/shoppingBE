...
app.post('/login',(req,res)=>{
  ...
  .then(data=>{
    ...
    res.send({mesage:'thanh cong',accessToken:accessToken})
  })
  .catch(()=>{
    res.send('that bai')
  })
...
app.get('/admin', async(req, res) => {
  ...
  const listDetailBill=await detailBill.find({})
  ...
  if(!accessToken){
    ...
    acount.find({_id:verify._id})
    .then((acount)=>{
      res.json({acount:acount,product:listProduct,bill:listBill,detail:listDetailBill})
    })
    .catch(()=>{
      res.send('Tai khoan hoac mat khau khong dung')
    })
